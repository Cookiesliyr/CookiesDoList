...
namespace Timer2 {
	public partial class Timer2: Form {
		...
		public void Timer_StopButt(object Sender, MouseEventArgs e) {
			...
			int TimerType = int.Parse(CurTProgra.TaskTabRAr[TabN][TaskN].TimersName[Ti][0].ToString());
			bool TF1 = ((TimerType&1)==1), TF2 = ((TimerType&2)==2);
			CurTProgra.TaskTabRAr[TabN][TaskN].TimersValue[Ti]=(TF2 ? CurTProgra.TaskTabRAr[TabN][TaskN].DefaultTimersValue[Ti] : 0);
			if (e.Button==MouseButtons.Left) CurTProgra.TaskTabRAr[TabN][TaskN].TimersRunning[Ti] = false;
		...
		public void GenerateTaskGUI(int TabN, int TaskN) {
			...
			for (int i = 0; i<CurTask.TimersName.Count; i++) {
				long TVal = CurTask.TimersValue[i];
				...
				TLAr[(TabN, TaskN)].Add(new Label { Text=TVal.ToString(), Visible=TF1, TextAlign=ContentAlignment.MiddleCenter });
			...
			foreach (TaskTabR TBK in CurTProgra.TaskTabRAr) {
				...
				foreach (TPTask TK in TBK.TaskAr) {
					for (int i =0; i < TK.TimersValue.Count; i++) {
						...
						TK.TimersValue[i] += 100;
						TPBAr[(TTTabN, TTTaskN)][i].Value=(int)(TK.TimersValue[i]/TK.DefaultTimersValue[i]*0.01);
						TLAr[(TTTabN, TTTaskN)][i].Text=TK.TimersValue[i].ToString();
					}
					TTTaskN++;
				}
